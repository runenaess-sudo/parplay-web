...
export default function Community({ courseId, initialCategory = '' }: { courseId?: string; initialCategory?: CommunityCategory | '' }) {
    const router = useRouter();
    const [user, setUser] = useState<string | null>(null);
    const [category, setCategory] = useState<CommunityCategory | ''>(courseId ? 'course' : initialCategory);
    ...
    useEffect(() => { if (category === 'course') return; let live = true; void (async () => {
    ...
    return <div className="mx-auto grid max-w-[1600px] grid-cols-1 gap-5 px-3 pb-12 text-white md:grid-cols-[240px_minmax(0,1fr)] xl:grid-cols-[270px_minmax(0,1fr)_300px] [&_button]:cursor-pointer [&_button:focus-visible]:outline-2 [&_button:focus-visible]:outline-emerald-300">
        <aside className={`${glass} self-start`}><h1 className="text-2xl font-semibold">Community</h1><p className="mt-2 text-sm leading-6 text-slate-300">Share experiences. Inspire others.<br />Together we make disc golf better.</p>
            <label className="mt-4 block md:hidden">Topic<select aria-label="Filter community topic" value={category} onChange={e => { if (courseId) { window.location.assign('/community?category=' + e.target.value); return; } setCategory(e.target.value as CommunityCategory | ''); setOffset(0); setLoading(true); setError(''); }} className="mt-2 w-full rounded-lg bg-[#173129] p-3"><option value="">All posts</option>{Object.entries(communityCategories).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></label>
            <nav aria-label="Community categories" className="my-4 hidden space-y-1 md:block">{[['', 'All posts'], ...Object.entries(communityCategories)].map(([key, label]) => <button key={key} aria-pressed={category === key} onClick={() => { if (courseId) { window.location.assign('/community?category=' + key); return; } setCategory(key as CommunityCategory | ''); setOffset(0); setLoading(true); setError(''); }} className={`block w-full rounded-lg border px-3 py-3 text-left text-sm ${category === key ? 'border-emerald-400/30 bg-emerald-400/10 font-semibold text-emerald-200' : 'border-transparent text-slate-300 hover:bg-white/5'}`}>{label}</button>)}</nav>
            {!courseId && <button onClick={() => { if (auth()) setShare(true); }} className="mt-4 w-full rounded-xl border border-emerald-400/40 bg-emerald-400/15 px-3 py-3 text-sm font-semibold text-emerald-100">+ Share your experience</button>}
        </aside>
        <section className="min-w-0 space-y-5" aria-label="Community feed">
            ...
            {category === 'course' ? <CourseFeed key={courseId || 'courses'} courseId={courseId} /> : <>
