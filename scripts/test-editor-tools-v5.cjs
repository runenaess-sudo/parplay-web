...
function panel() {
 ...
 for (let i = 1; i <= 18; i++) {
  ...
  data.configs.push({ id: `config-${i}`, hole_id: `hole-${i}`, tee_id: `tees-${i}`, basket_id: `baskets-${i}`, par: 3, distance: 83 });
 }
 data.configs.push({ ...data.configs[0], id: 'blue', tee_id: 'tees-2' });
 ...
 const calls = []; const warnings = []; let confirm = true; let cursor = 0; const slots = []; const effects = [];
 ...
 vm.runInNewContext(ts.transpileModule(read('app/create-course/editor/[courseId]/PlayOptionsPanel.tsx'), { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022, jsx: ts.JsxEmit.ReactJSX } }).outputText,
  { exports, require: name => { assert.ok(name in mocks, name); return mocks[name]; }, crypto: { randomUUID: () => 'new-id' }, window: { confirm: message => { warnings.push(message); return confirm; } } });
 let tree;
 ...
 render();
 return { data, calls, warnings, render, all, text, store: () => store, reject: () => { confirm = false; },
...
test('18-hole course normal panel renders only active objects, no coordinates, separate par summary', async () => {
 ...
 const cards = p.all(n => n.type === 'summary').map(p.text).filter(x => x.includes('Current option'));
 assert.equal(cards.length, 2); assert.ok(cards[0].startsWith('tees-1')); assert.ok(cards[1].startsWith('baskets-1'));
 ...
 const options = p.all(n => n.props['aria-label'] === 'Play option')[0]; assert.equal(p.all(n => n.type === 'option', options).length, 2);
...
test('existing basket selector exposes course-wide usage; selection/Cancel do not persist', async () => {
 const p = panel(); await p.settle(); p.click('Use existing basket');
 const selector = p.all(n => n.props['aria-label'] === 'Existing basket')[0];
 assert.equal(p.all(n => n.type === 'option', selector).length, 19); assert.match(p.text(selector), /baskets-18 · Holes 18/);
...
test('selected shared basket move still warns, Cancel does not write', async () => {
 const p = panel(); await p.settle(); p.reject(); p.click('Move to map position', 1);
 assert.match(p.warnings[0], /all 2 current play options/); assert.match(p.warnings[0], /Played rounds remain unchanged/);
 assert.equal(p.calls.filter(c => c.name.startsWith('save_')).length, 0);
});

test('switching play option uses resolved applicability; sharing targets active config', async () => {
 const p = panel(); await p.settle();
 p.all(n => n.props['aria-label'] === 'Play option')[0].props.onChange({ target: { value: 'blue' } }); await p.settle();
